...
TEST_CASE("Write into a string") {
    check_write_stream<neo::dynbuf_stream<neo::proto_dynamic_buffer>>();

    std::string        str;
    neo::dynbuf_stream strm(neo::as_dynamic_buffer(str));
...
TEST_CASE("Read from a string") {
    check_read_stream<neo::dynbuf_stream<neo::proto_dynamic_buffer>>();
